...
Public Class Tortilleriafrm
    Dim CargaDatos As DataSet
    Dim DatosAdapter As MySqlDataAdapter
    Private Sub Tortilleriafrm_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        e.Cancel = False
        CargarTortilleriaFrm.Dispose()
    End Sub
    Private Sub Tortilleriafrm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ConectaServer()
        CargarConfig()
        Me.Text += Ayuntamiento & " : TORTILLERIA MUNICIPAL"
    ...
    Public Sub CargarIngresos()
        ...
        DatosAdapter.Fill(CargaDatos, "tortilleria")
        ...
        Me.DatosDtg.DataMember = "tortilleria"
        'Dim c As Short
    ...
    Private Sub TodosBtn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TodosBtn.Click
        SQL = "SELECT * FROM tortilleria"
        Me.TituloLbl.Text = "TODOS LOS INGRESOS DE LA TORTILLERIA MUNICIPAL"
        CargarIngresos()
    End Sub
    ...
    Private Sub IngMesCmb_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles IngMesCmb.SelectedIndexChanged
        ...
        SQL = "SELECT * FROM tortilleria where fecha Between '" & Now.Year & "-" & NoMes & "-01" & "' and '" & Now.Year & "-" & NoMes & "-" & DiasMes & "' order by fecha asc"
        CargarIngresos()
        Me.TituloLbl.Text = "TODOS LOS INGRESOS DE LA TORTILLERIA MUNICIPAL REGISTRADOS EN EL MES DE " & Me.IngMesCmb.Text.ToUpper & " DEL AÑO " & Now.Year
    End Sub
    ...
    Private Sub IngDiaDtp_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles IngDiaDtp.ValueChanged
        SQL = "SELECT * FROM tortilleria where fecha ='" & Me.IngDiaDtp.Value.Year & "-" & Me.IngDiaDtp.Value.Month & "-" & Me.IngDiaDtp.Value.Day & "' order by id asc"
        CargarIngresos()
        Me.TituloLbl.Text = "TODOS LOS INGRESOS DE LA TORTILLERIA MUNICIPAL REGISTRADOS EL DIA " & Me.IngDiaDtp.Value.ToLongDateString.ToUpper
    End Sub
    ...
    Private Sub IngDelAnoCmb_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles IngDelAnoCmb.SelectedIndexChanged
        SQL = "SELECT * FROM tortilleria where fecha like '%" & Me.IngDelAnoCmb.Text & "%'"
        Me.TituloLbl.Text = "TODOS LOS INGRESOS DE LA TORTILLERIA MUNICIPAL REGISTRADOS EN EL AÑO " & Me.IngDelAnoCmb.Text
        CargarIngresos()
    End Sub
    ...
    Private Sub ImprimirBrn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ImprimirBrn.Click
        If Me.DatosDtg.Rows.Count > 0 Then
            ...
            For i = 0 To Me.DatosDtg.Rows.Count - 1
                ReporteFrm.Datos.Add(New DatosTortilleriaCls(Me.DatosDtg.Item(1, i).Value.ToString, Me.DatosDtg.Item(2, i).Value.ToString, Me.DatosDtg.Item(3, i).Value.ToString, Mid(Me.DatosDtg.Item(4, i).Value.ToString, 1, 10)))
            Next
            ReporteFrm.Show()
        Else
            MessageBox.Show("No se encontró nada que imprimir", "Tortilleria Municipal", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End If
    End Sub
End Class